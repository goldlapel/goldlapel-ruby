...
module GoldLapel
  ...
  # --
  # Phase 5 Redis-compat families (async sibling of GoldLapel.* helpers).
  # Same patterns / contracts as goldlapel/utils.rb, calling
  # `async_exec_params` instead of `exec_params`.
  # --
  ...
  # Proxy contract: $1=lon, $2=lat, $3=radius_m, $4=limit. CTE-anchor in the
  # proxy SQL collapses the lon/lat to a single `$N` reference each — pass a
  ...
  # Proxy contract: $1=member, $2=member, $3=radius_m, $4=limit (anchor
  # member is $1 for the join, $2 for the self-exclusion; with native $N
  ...
  def self._require_patterns(patterns, fn)
    unless patterns && patterns[:query_patterns]
      raise RuntimeError,
        ...
        "`gl.#{fn}(...)` rather than the module function directly."
    end
