...
module GoldLapel
  ...
  # Returns the FQ table (e.g. "_goldlapel.doc_users"). The wrapper API never
  # builds CREATE TABLE itself anymore — `gl.documents.<verb>(...)` always
  # supplies `patterns` from the proxy DDL API (see goldlapel/ddl.rb).
  ...
  # `lookup_tables` maps each $lookup.from collection name → its canonical
  # proxy table (e.g. {"orders" => "_goldlapel.doc_orders"}). DocumentsAPI
  # builds this map by walking the pipeline and fetching DDL for each unique
  # `from` reference (cached after first call). When omitted, raw `from`
  # collection names are used — kept only as a fallback for direct callers.
  def self.doc_aggregate(conn, collection, pipeline, patterns: nil, lookup_tables: nil)
    ...
    # Build $lookup subqueries. `from` is the user-facing collection name; the
    # actual SQL references its canonical proxy table (resolved from
    # lookup_tables, populated by DocumentsAPI#aggregate). The validated
    # `from` is reused as a SQL alias so `from.data->>'…'` stays unambiguous
  ...
  # `gl.documents.create_collection(name, unlogged:)` invokes the proxy DDL
  # via `DocumentsAPI#_patterns`; by the time this method is called, the
  # canonical table has already been materialized on the proxy side. The
  # `unlogged:` and other creation-time options are properties of `patterns`
  # (and of the proxy DDL fetch that produced them) — passing them here is
  # accepted for signature stability but ignored.
