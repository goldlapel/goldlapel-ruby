...
class TestUnsafeGucClassifier < Minitest::Test
  ...
  def test_safe_gucs_are_safe
    %w[timezone application_name statement_timeout work_mem
       client_encoding DateStyle].each do |name|
...
class TestParseSetCommand < Minitest::Test
  ...
  def test_rejects_set_time_zone_two_word_form
    # Legacy two-word form. Timezone is harmless; treating it as
    # not-a-trackable-SET is correct (cache-safe).
    assert_nil parse("SET TIME ZONE 'UTC'")
  end
end
...
class TestConnectionGucState < Minitest::Test
  ...
  def test_observe_sql_returns_change_flag
    ...
    refute @s.observe_sql("SET timezone = 'UTC'"), "safe SET ⇒ unchanged"
